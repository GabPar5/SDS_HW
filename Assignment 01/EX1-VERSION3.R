...
std_dev <- sqrt(N * p * (1 - p)) # calculate the standard deviation

# score function for each students
score_function <- function(N, p, q, std_dev, alpha) {
  
  # set the threshold at mean + 2 standard deviations
  T <- N * p + 2 * std_dev
...
scores <- replicate(total_students, score_function(N, p, q, std_dev, alpha))
